...
def db_add_new_user(username, password):
    try:
        user = Users(username=username, password=password, intro='', avatar='')
        db.session.add(user)
        db.session.commit()
        return True
...
def db_get_user_info(username):
    try:
        cursor = db.session.execute("SELECT intro, avatar FROM users WHERE username = '{}';".format(username))
        for cur in cursor:
            return True, {'intro': cur[0], 'avatar': cur[1]}
